	...
	def duplicate_am_continuity(df, clinic_name, special_cases=None):
	    ...
	    if df is not None and not df.empty:
	        ...
	        if clinic_name in special_cases:
	            ...
	            df = pd.concat([df, pm_continuity_rows], ignore_index=True) #df = pd.concat([df, pm_continuity_rows], ignore_index=True).sort_values(by=["date", "provider"]).reset_index(drop=True)
	            ...
	            df = pd.concat([df, df, pm_continuity_rows, pm_continuity_rows], ignore_index=True) #df = pd.concat([df, df, pm_continuity_rows, pm_continuity_rows], ignore_index=True).sort_values(by=["date", "provider"]).reset_index(drop=True)
