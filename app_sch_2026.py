...
if mode == "OPD Check":
    ...
    def build_maps_and_roster(df: pd.DataFrame):
        ...
        Notes:
          - IGNORE entries that have no student after the tilde (e.g., "Lastname, Firstname ~") for double-book detection.
            They still contribute to the weekly roster so they can show up as available.
          - IGNORE placeholder/administrative rows (e.g., Closed, Vacation, Admin, Notes).
        """
            ...
            for col_idx, day in enumerate(DAYS, start=1):
                ...
                for row in range(rstart, rend+1):
                    ...
                    if not pre or is_placeholder_preceptor(pre):
                        continue
                    # roster is for availability pool
                    roster[(monday_date, period)].add(pre)
                    # keep booking only if real student AND valid date
                    if stu is None:
                        continue
