...
if st.session_state.page == "Home":
	...
	# Assume unique_student_names is already defined.
	options = ['Week 1', 'Week 2', 'Week 3', 'Week 4']
	
	# Multi-select widget outside of any form, with a fixed key.
	...
	if selected_students:
	    ...
	    # Loop over the selected students
	    for student in selected_students:
	        # Create a unique session key for each student's week selection.
	        session_key = f"week_{student}"
	        # If there's no stored value, initialize it with a default (e.g. "Week 1")
	        if session_key not in st.session_state:
	            st.session_state[session_key] = "Week 1"
	        # Determine the default index based on the stored value.
	        ...
	        selected_week = st.selectbox(
	            ...
	            options,
	            index=default_index,
	            key=session_key  # this key will store the current value automatically
	        )
	        assignments[student] = selected_week
	
	    # When the button is pressed, process the assignments.
	...
	if st.session_state.assignment_done:
	    # -----------------------------
	    # WARD_A Assignment (Evenly Distributed)
	    # -----------------------------
	    weeks = {
	    ...
	    # Define available room pairs and capacity.
	    ward_a_room_pairs = [("H0", "H10"), ("H1", "H11"), ("H2", "H12"), ("H3", "H13"), ("H4", "H14")]
	    ward_a_capacity = 5  # maximum assignments per week
	    ...
	    # Assign WARD_A for students not already assigned.
	    for student in student_names:
	    ...
	    # -----------------------------
	    # Extra Assignments for HAMPDEN_NURSERY, SJR_HOSP, PSHCH_NURSERY
	    # -----------------------------
	    ...
	    def assign_slot(student, clinic, week, room_pair):
	        """Attempt to assign 'student' to a slot for 'clinic' in the given week."""
	    ...
	    # Pre-check for extra slots already assigned.
	    ...
	    for student in student_names:
	        ...
	        # Priority 1: HAMPDEN_NURSERY (only week1 & week3)
	        ...
	        # Priority 2: SJR_HOSP (allowed in any week except WARD_A week)
	        ...
	        # Priority 3: PSHCH_NURSERY (allowed in any week except WARD_A week)
	        if not assigned_extra:
	    ...
	    # ----------------------------------------------------------------
	    # (Additional processing: duplicate check, outpatient shift analysis, etc.)
	    # ----------------------------------------------------------------
	    ...
	    # ----------------------------------------------------------------
	    # Outpatient Shift Analysis and Final Excel Mapping
	    # ----------------------------------------------------------------
	    ...
	    shift_counts = (filtered_df.groupby(["week_label", "provider", "clinic"]).size()
	    ...
	    pivot_df = grouped_df.pivot(index="student", columns="week_label", values="clinic").reset_index()
	    pivot_df.columns.name = None
	    pivot_df = pivot_df.rename_axis(None, axis=1)
	    st.dataframe(pivot_df)
	
	    # ----------------------------------------------------------------
	    # OPD Excel Mapping and Download
	    # ----------------------------------------------------------------
	    ...
	    def process_excel_mapping(location, sheet_name):
	        ...
	        for row in ws.iter_rows():
	            t_value = row[7].value  # Column H
	            h_value = row[6].value  # Column G
	            row_location = row[4].value  # Column E
	            ...
	            if row_location == location and t_value in combined_t_mapping and h_value in combined_t_mapping[t_value]:
	                ...
	                ws1.cell(row=target_row, column=target_column).value = row[5].value  # Column F
	                ws1.cell(row=target_row, column=target_column).alignment = Alignment(horizontal='center')
	    
	        wb1.save('OPD.xlsx')
	        st.write(f"Processed mapping for {location} in {sheet_name}.")
