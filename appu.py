...
if st.session_state.page == "Home":
	...
	# ============================================================
	# Pre-requisites:
	# Assume that 'df' is your DataFrame containing rows for each clinic
	# with columns "clinic", "datecode", "class", etc.
	# (The "student" column is assumed blank already.)
	# ============================================================
	...
	for student in student_names:
	    ...
	    for room_pair in ward_a_room_pairs:
	        ...
	        if df.loc[mask, "student"].isna().any():
	            # Fill all matching rows with the student's name.
	...
	# (Optionally, inspect ward_a_assignment)
	
	# ============================================================
	# STEP 2: MANUAL ASSIGNMENT for PSHCH_NURSERY Extra Assignment
	# ============================================================
	st.header("Manual PSHCH_NURSERY Extra Assignment")
	manual_week = st.selectbox("Select week for manual PSHCH_NURSERY assignment", ["week1", "week2", "week3", "week4"])
	manual_students = st.multiselect("Select student(s) to manually assign to PSHCH_NURSERY", unique_student_names)
	...
	if st.button("Run Manual PSHCH_NURSERY Assignment"):
	    for student in manual_students:
	        ...
	        elif assign_slot(student, "PSHCH_NURSERY", manual_week, pshch_fallback):
	            assigned = True
	            manual_assignment[student] = ("PSHCH_NURSERY", manual_week, pshch_fallback)
	        if not assigned:
	            st.error(f"Could not assign {student} manually to PSHCH_NURSERY in {manual_week}.")
	    st.write("Manual PSHCH_NURSERY assignments:", manual_assignment)
	
	# ============================================================
	# STEP 3: EXTRA ASSIGNMENT for Remaining Students (Skip those manually assigned)
	# ============================================================
	...
	extra_assignment = {}
	manual_assigned = set(manual_assignment.keys())
	
	for student in student_names:
	    if student not in ward_a_assignment:
	        continue  # Should not happen if all students got WARD_A.
	    if student in manual_assigned:
	        continue  # Skip students manually assigned.
	    ...
	    # Priority 2: SJR_HOSP (in any week except the student's WARD_A week)
	    ...
	    # Priority 3: PSHCH_NURSERY (if not manually assigned)
	    if not assigned_extra:
	        for wk in ["week1", "week2", "week3", "week4"]:
	            if wk == wa_week:
	                continue
