...
# Upload Files Page
	...
	consults_df = warda_df[warda_df["type"].isin(["consultsp ", "consultsa "])].assign(type=lambda df: df["type"].map({"consultsp ": "PM - Continuity ", "consultsa ": "AM - Continuity "}), clinic="ER_CONS").groupby(["date", "clinic", "type"], as_index=False).agg({"provider": lambda x: " / ".join(x.astype(str)) + " ~" if x.name == "PM - Continuity " else x.str.strip()})
