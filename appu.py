...
# Upload Files Page
	...
	consults_df = warda_df[warda_df["type"].isin(["consultsp ", "consultsa "])].assign(type=lambda df: df["type"].map({"consultsp ": "PM - Continuity ", "consultsa ": "AM - Continuity "}), clinic="ER_CONS"); consults_df = consults_df.groupby(["date", "clinic"], as_index=False).agg({"provider": lambda x: " / ".join(x.dropna().astype(str)) + " ~" if x.name == "PM - Continuity " else x.iloc[0]})
